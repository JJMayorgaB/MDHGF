...
# Usar la primera u para definir el rango de omega
u_ref = u_vals[0]
m1_ref = v - u_ref
omega_vals = np.linspace(-5 * m1_ref, 5 * m1_ref, N_omega)
eta = 1.0 * (omega_vals[1] - omega_vals[0])

x_vals     = np.linspace(-x_max+x0, x_max+x0, N_x)

print("Configurando grilla de frecuencias ...")
...
def compute_rho_total(u_param):
    ...
    for i_o, omega in enumerate(omega_vals):
        omega_c = omega + 1j * eta
        g00 = green_r(omega_c, x0, x0, u_param)
        ...
        for i_x, x in enumerate(x_vals):
            dg = delta_green_r(omega_c, x, x0, M_inv, u_param)
...
def update(frame_idx):
    ...
    m1_param = v - u_param
    print(f"  Frame {frame_idx+1}/{len(u_vals)}  (u = {u_param:.2f}, m1 = {m1_param:.2f})")

    rho_total = compute_rho_total(u_param)
    
    # Calcular rho0 para poder restar
    ...
    title.set_text(
        f'LDOS con T={tipo}, U0={U0:.0f}, u={u_param:.2f}, v={v:.1f}, a={a:.1f}, fase={phase}'
    )
...
if formato == 'gif':
    ...
    print("Generando GIF ...")
    anim.save(out_file, writer='pillow', fps=5, dpi=150)
elif formato == 'mp4':
